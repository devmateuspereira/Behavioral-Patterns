...
public abstract class BestOfferTemplate {
    ...
    protected Double calculateDeliveryTax() {
        ...
        Double totalWeight = cart.getItems().stream().reduce(0.0, (acc, item) -> acc + item.getWeight(), Double::sum);
        return (distance * totalWeight) / 100;
    }

    protected Double calculateRegularItemsPrice() {
        return cart.getItems().stream().reduce(0.0, (acc, item) -> acc + item.getValue(), Double::sum);
    }
